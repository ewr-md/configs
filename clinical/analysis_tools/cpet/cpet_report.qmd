...
source(here::here('cpet_values.R'))
```
...
Resting tidal volume was `r VTex_rest` liters. This increased to `r VTex_max` liters at peak exercise, which is a(n) `r status_ve_response(VTex_rest, VTex_max) ` response. Respiratory rate at rest was `r rr_rest`/min. This increased to `r rr_max`/min at peak exercise, which is a(n) `r status_rr_response(rr_rest, rr_max)` response.
...
Maximal PETCO2 was `r status_petco2(PETCO2)` at `r PETCO2` mmHg.
VE/VCO2 slope was `r status_slope_veco2(slope_veco2)`.
